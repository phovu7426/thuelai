...
function initFloatingSocial() {
    const socialContainer = document.getElementById("floatingSocial");
    ...
    // Auto hide menu after 10 seconds
    ...
    if (zaloBtn) {
        zaloBtn.addEventListener("click", () => {
            ...
            const message = encodeURIComponent(
                "Xin chào! Tôi muốn tư vấn dịch vụ lái xe thuê từ website ThuêLai.vn"
            );
    ...
    document.addEventListener("click", (e) => {
        if (!socialContainer.contains(e.target) && isMenuOpen) {
            socialMenu.classList.remove("show");
            isMenuOpen = false;
            clearTimeout(hideTimeout);
        }
    ...
    // Reset hide timer when hovering over menu
    socialMenu.addEventListener("mouseenter", () => {
        clearTimeout(hideTimeout);
    });

    socialMenu.addEventListener("mouseleave", () => {
        if (isMenuOpen) {
            startHideTimer();
        }
    });
