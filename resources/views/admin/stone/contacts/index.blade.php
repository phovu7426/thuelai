                                ...
                                <tbody>
                                    @foreach ($contacts as $contact)
                                        <tr class="{{ $contact->is_read ? '' : 'table-warning' }}">
                                            ...
                                            <td>
                                                <a href="{{ route('admin.stone.contacts.show', $contact->id) }}"
                                                    class="action-icon">
                                                    <i class="mdi mdi-eye"></i>
                                                </a>
                                                <a href="javascript:void(0);" class="action-icon delete-contact"
                                                    data-id="{{ $contact->id }}">
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                $('.contact-checkbox:checked').each(function() {
                                    ...
                                    $(this).closest('tr').find('.badge').removeClass(
                                        'bg-warning').addClass('bg-success').text(
                                        'Đã đọc');
                                });

                                // Cập nhật số lượng chưa đọc
                                const unreadCount = parseInt("{{ $unreadCount }}") -
                                    selectedIds.length;
                                if (unreadCount > 0) {
                                ...
                                // Hiển thị thông báo
                                toastr.success('Đã đánh dấu ' + selectedIds.length +
                                    ' liên hệ là đã đọc');
                            }
                        }
                    });
