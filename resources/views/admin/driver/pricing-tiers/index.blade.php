                                            ...
                                            <tr data-id="{{ $tier->id }}">
                                                ...
                                                <td>
                                                    <div class="form-check form-switch">
                                                        <input class="form-check-input toggle-highlight" type="checkbox" 
                                                               data-id="{{ $tier->id }}"
                                                               {{ $tier->is_highlighted ? 'checked' : '' }}>
                                                        <label class="form-check-label highlight-label-{{ $tier->id }}">
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // Cập nhật label
                    $(`.status-label-${tierId}`).text(isChecked ? 'Hoạt động' : 'Tạm dừng');
                } else {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    showAlert('success', response.message);
                    // Cập nhật label
                    $(`.highlight-label-${tierId}`).text(isChecked ? 'Nổi bật' : 'Không nổi bật');
                } else {
                    showAlert('danger', response.message);
                    // Revert checkbox
                    $(this).prop('checked', !isChecked);
